...
description: "Turn a Jira issue into a GitHub-native staggered PR train: split refactors, behavior changes, tests, and docs into stacked branches and draft PRs that merge bottom-up into one handoff feature branch instead of directly to main."
---
...
Create a GitHub-native stacked PR train whose final landing target is a single handoff feature branch for the Jira issue.
...
main or default branch
 └─ feature/<JIRA-KEY>-<short-slug>                  # handoff branch, do not review directly
...
After the train is reviewed and merged bottom-up, the handoff feature branch contains the complete Jira implementation and can be handed to a human colleague. Do not open or merge directly to `main`, `master`, `develop`, or the repository default branch unless the user explicitly asks.
...
6. The bottom PR in the stack targets the handoff feature branch, not the repository default branch.
...
10. If the repository has an `AGENTS.md`, `CONTRIBUTING.md`, PR template, branch naming convention, or CI instructions, follow them unless they conflict with this skill's safety and reviewability rules.
...
```text
Jira: <KEY> <title>
Handoff branch: feature/<KEY>-<slug>
Train:
1. train/<KEY>/01-... -> feature/<KEY>-<slug>
   Scope: ...
...
git switch -c "feature/<KEY>-<slug>" "origin/${DEFAULT_BRANCH}"
git push -u origin "feature/<KEY>-<slug>"

git switch -c "train/<KEY>/01-<layer-slug>" "feature/<KEY>-<slug>"
...
Final handoff branch: feature/<KEY>-<slug>
...
```text
Merge order:
1. Review and merge PR 1 into feature/<KEY>-<slug>.
2. Retarget PR 2 to feature/<KEY>-<slug> if GitHub does not do so automatically.
3. If the repository squash-merges stacked PRs, rebase PR 2 with:
   git fetch origin
...
5. Repeat until the train is complete.
6. At the end, feature/<KEY>-<slug> is the complete handoff branch.
```

Prefer merge commits for intermediate train merges when the repository allows them, because they preserve ancestry and make stacked branch retargeting easier. If the repository requires squash merges, rebase the next branch after each merge using `git rebase --onto` as shown above.
